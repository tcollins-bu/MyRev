...
public class TestFeedService {
	 ...
	 * 
	 * @Test public void testMock() { ThreadRepository mockObj =
	 * Mockito.mock(ThreadRepository.class); List<Thread> threads =
	 * threadRepository.findAll();
	 * Mockito.when(mockObj.findAll()).thenReturn(threads);
	 * 
	 * List<Thread> mockThreads = mockObj.findAll();
	 * 
	 * // Assert. assertEquals(threads, mockThreads);
	 * Mockito.verify(mockObj).findAll(); }
	 * 
	 * @Test final void testFindById() {
	 * when(threadRepository.findById(1)).thenReturn(thread); }
	 * 
	 * 
	 * @Test public void threadFindByIdMock() { ThreadRepository
	 * localMockThreadRepository = Mockito.mock(ThreadRepository.class);
	 * JpaRepository<Thread, Integer> threadRepository = null ;//new
	 * JpaRepository<Thread, Integer>();; // ThreadRepository threadRepository = new
	 * ThreadRepository(); List<Thread> threads = threadRepository.findAll();
	 * 
